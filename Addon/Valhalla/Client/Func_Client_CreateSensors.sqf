	...
	//_sensor=createTrigger["EmptyDetector",[0,0]];
	//_sensor setTriggerActivation ["ANY", "PRESENT", true];
	//_sensor setTriggerStatements["(local player) && (alive player) && (Local_PlayerVehicle==player) && ((player distance nearestObject[player,""LandVehicle""])<10) && (alive nearestObject[player,""LandVehicle""]) && !(isNull nearestObject[player,""LandVehicle""]) && ((({_x in System_FieldRepirPacks} count (weapons _target)))<1) && !(nearestObject[player,""LandVehicle""] in Local_UserVehicles)","[""LandVehicle""] call Func_Client_AddLockActions",""];
