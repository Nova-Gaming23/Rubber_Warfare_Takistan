	...
	{					
		...
		_action=Local_PlayerVehicle  addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_LowGearOff")+"</t>","Addon\Valhalla\Client\Script_Client_Actions.sqf",[28],91,false,true,"","(isPlayer _this) && (player==driver _target) && (Local_HighClimbingModeOn) && (canMove _target)"];
		Local_PlayerVehicle setVariable ["DAO_WF_ACT_LowGearOff",_action];
		_action=Local_PlayerVehicle  addAction ["<t color='#FFBD4C'>"+(localize "STR_ACT_LowGearOn")+"</t>","Addon\Valhalla\Client\Script_Client_Actions.sqf",[28],91,false,true,"","(isPlayer _this) && (player==driver _target) && (!Local_HighClimbingModeOn) && (({_target isKindOf _x} count System_TrackedVehicleTypes)>0)"];
