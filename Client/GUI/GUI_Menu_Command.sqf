...
while {alive player && dialog} do {
	...
	//GET PEACETIME & SpawnProteckt INFO for SqadRespawn  //--by NG23
	_sortDis = [(leader _team),towns] Call SortByDistance;
	_nearestTown = _sortDis Select 0;
	_enteredTown = (((leader _team) distance _nearestTown) < 600);
	_isInPT = if (isNil {_nearestTown getVariable "peacemc"}) then {false}else{true};
	_sideTownID = _nearestTown getVariable "sideID";
	_SpawnSafeZone = if (!((leader _team) in (list SpawnSafeZone_1)) && !((leader _team) in (list SpawnSafeZone_2))) then {false}else{true};
	//End GET
	if (_updateTab) then {
		_currentIDC = 0;
		switch (_mode) do {
	...
	if (MenuAction == 1) then {
		...
		_special = if (_isAll) then {false} else {
						if (!(isPlayer(leader _team)) && !(_SpawnSafeZone)) then {true} else {
							if ((isPlayer(leader _team)) && _sideTownID != sideID && _enteredTown && _isInPT) then {
								true}else{false};};};
	...
	if (MenuAction == 201) then {
		...
		if (isPlayer(leader _team)) then {		
			...
			WFBE_LocalizeMessage = [sideJoined,'CLTFNCLOCALIZEMESSAGE',['PlayerPeaceTimeRespawn',name (leader _team)]];
			publicVariable 'WFBE_LocalizeMessage';
			['PlayerPeaceTimeRespawn',name (leader _team)] Spawn CLTFNCLocalizeMessage;
		};
			_units = Units _team;
			if (PARM_Respawn_Mobile >= 2 || (('WFBE_RESPAWNCAMPSMODE' Call GetNamespace) > 0)) then {//--- Make sure that the unit won't spawn back at a camp/ambu.
