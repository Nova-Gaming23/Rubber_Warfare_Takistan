...
if (PARM_Equipment_Artillery == 0) then {{ctrlEnable [_x,false]} forEach _IDCS};
...
//--- Support List.
_lastSel = -1;
_addToList = [localize 'STR_WF_TACTICAL_FastTravel',localize 'STR_WF_ICBM',localize 'STR_WF_TACTICAL_ParadropAmmo',localize 'STR_WF_TACTICAL_ParadropVehicle',localize 'STR_WF_TACTICAL_UnitCam',localize 'STR_WF_TACTICAL_UAV',localize 'STR_WF_TACTICAL_UAVDestroy',localize 'STR_WF_TACTICAL_UAVRemoteControl'];
_addToListID = ["Fast_Travel","ICBM","Paradrop_Ammo","Paradrop_Vehicle","Units_Camera","UAV","UAV_Destroy","UAV_Remote_Control"];
_addToListFee = [0,300000,6000,51000,0,26500,0,0];
_addToListInterval = [0,100,180,180,0,0,0,0];
...
if (PARM_Equipment_Artillery == 0) then {
...
while {alive player && dialog} do {
	...
	if (MenuAction == 20) then {
		...
		switch (_currentSpecial) do {
			...
			case "UAV": {
				...
				RUBHUD = false;
				RUBGPS = 0;
				null = [1]ExecVM "Client\Module\UAV\uav.sqf";
			};
			case "UAV_Destroy": {
	...
	//--- Artillery List.
	if (PARM_Equipment_Artillery == 1 && (_requestRangedList || time - _lastArtyUpdate > 3)) then {
