...
for [{_i = 0},{_i < (count (missionConfigFile/"Params"))},{_i = _i + 1}]  do {
	...
	_row = lnbAddRow [22003,[_text,_status]];
	if (isMultiplayer) then {
		if (_value != getNumber(missionConfigFile >> "Params" >> _paramName >> "default")) then {
			lnbSetColor [22003, [_row, 0], [0.78,0.4,0.141,1]];
			lnbSetColor [22003, [_row, 1], [0.78,0.4,0.141,1]];
		};		
	};
};
