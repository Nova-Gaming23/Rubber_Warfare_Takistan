	...
	if (MenuAction == 1) then {
		...
		if ((_currentRow) != -1) then {
			...
			if !(_skip) then {
				...
				//--- Get the infantry limit based off PLAYER RANK
				/*_realSize = ((sideJoined) Call GetSideUpgrades) select 0;
				switch (_realSize) do {
					case 0: {_realSize = (_mbu - 1)};
					case 1: {_realSize = _mbu };
					case 2: {_realSize = _mbu };
					case 3: {_realSize = (_mbu + 1)};
					default {_realSize = _mbu};
				};*/
				_realSize = _mbu;
				_score = score player;
				if (parmAddAIbyRank) then {
				if (_score < 10 ) then {_realSize = _mbu};
				if (_score >= 10 ) then {_realSize = (_mbu + 1)};
				if (_score >= 30 ) then {_realSize = (_mbu + 2)};
				if (_score >= 50 ) then {_realSize = (_mbu + 3)};
				if (_score >= 75 ) then {_realSize = (_mbu + 4)};
				if (_score >= 100 ) then {_realSize = (_mbu + 5)};
				if (_score >= 150 ) then {_realSize = (_mbu + 6)};
				if (_score >= 200 ) then {_realSize = (_mbu + 7)};
				if (_score >= 300 ) then {_realSize = (_mbu + 8)};
				};
