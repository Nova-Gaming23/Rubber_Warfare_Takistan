...
lnbClear _listBox;
{
	_addin = true;
	if (_filter != "nil") then {
		if (((_x Call GetNamespace) select QUERYUNITFACTION) != _filter) then {_addin = false};
	};
	if (((_x Call GetNamespace) select QUERYUNITUPGRADE) <= (_currentUpgrades select _value) && _addin) then {
		lnbAddRow [_listBox,['$'+str ((_x Call GetNamespace) select QUERYUNITPRICE),((_x Call GetNamespace) select QUERYUNITLABEL)]];
