...
if (!isNull playerUAV) exitWith {
	if (WF_A2_Vanilla) then {
		...
		null = [_mma]ExecVM "Client\Module\UAV\uav_interface_oa.sqf";
	};
};
...
//_uav = createVehicle [Format ["WFBE_WESTUAV",sideJoinedText] Call GetNamespace,getPos _closest, [], 0, "FLY"];
playerUAV = _uav;
Call Compile Format ["_uav addEventHandler ['Killed',{[_this select 0,_this select 1,%1] Spawn UnitKilled}]",sideJoined];
_uav SetVehicleInit Format["[this,%1] ExecVM 'Common\Common_InitUnit.sqf';this flyinheight 1000;",sideJoined];
processInitCommands;


//--- Remove weapons if air restriction is enabled.
if (PARM_Equipment_Planes == 0) then {removeAllWeapons _uav};
...
while {alive _uav} do {
	...
	for "_d" from 0 to (360-_step) step _step do
	{
		...
		_wp = (group _uav) addwaypoint [_pos,50];
		_wp setWaypointType "MOVE";
		_wp setWaypointBehaviour "CARELESS";
		_wp setwaypointdescription ' ';
		_wp setwaypointcompletionradius (1250/_wpcount);
	};
	...
	_spawn = [_uav,_add,_step,_lastWPpos,_radius,_dir] spawn {
		...
		while {alive _uav} do {
			...
			_wp setWaypointType "MOVE";
			_wp setWaypointBehaviour "CARELESS";
			_wp setwaypointdescription ' UAV Wp';
