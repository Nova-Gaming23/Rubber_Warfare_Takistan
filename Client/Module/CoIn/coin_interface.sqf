...
_tooFar = false;
if (PARM_Base_BaseAreaLimit != -1) then {
	if (_source == _hq) then {
		if (count(WF_Logic getVariable Format ['%1Area',SideJoinedText]) >= PARM_Base_BaseAreaLimit) then {
...
BIS_CONTROL_CAM_Handler = {
	...
	if (_mode == "keydown") then {
		...
		//--- Help Menu (F1)
		if (_key == 0x3B) then {
			if (isNil 'RscMenu_Help_SingelInfo') then {
// Help menu content
_string_HelpMenu = Format["
<br/>
<t align='left' size='1.3'> --- Controls --- </t><br/>
<t align='left' size='1'><br/>
<br/>
<br/>
Night vision = %1 (NightVision)<br/>
Rebuild last defense object = %2 (Use Action 15)<br/>
Auto Defenses = %3 (Use Action 16)<br/>
Sell Defense = %4 (Use Action 17)<br/></t>
<t align='left' size='0.8'><br/>
<br/>
<br/>
Where can i change the controls´s?<br/>
    Go to: ArmA2-Options > Controls > (Look for NightVision or UserAction)<br/>
<br/>
Where to find UserAction Controls?<br/>
    In ArmA2 Controls windows look for DropDown-Menu and select 'Custom controls'.<br/>
<br/>
</t>
",
if (actionKeysNames "NightVision" == "") then {"<t color='#e0963a'>Not set</t>"}else{Format["<t color='#76ba3c'>%1</t>",actionKeysNames "NightVision"]},
if (actionKeysNames "User15" == "") then {"<t color='#e0963a'>Not set</t>"}else{Format["<t color='#76ba3c'>%1</t>",actionKeysNames "User15"]},
if (actionKeysNames "User16" == "") then {"<t color='#e0963a'>Not set</t>"}else{Format["<t color='#76ba3c'>%1</t>",actionKeysNames "User16"]},
if (actionKeysNames "User17" == "") then {"<t color='#e0963a'>Not set</t>"}else{Format["<t color='#76ba3c'>%1</t>",actionKeysNames "User17"]}
];

				RscMenu_Help_SingelInfo = [Localize "STR_WF_BuildMenu", _string_HelpMenu];
				createDialog "RscMenu_Help_SingelInfo";			
			};			
		};
		
		//--- Manning Defense (Custom Action #2).
		if ((_key in (actionKeys "User16")) && (PARM_Base_AutoDefenses > 0)) then {
			
			switch (str manningDefense) do {
				case "false":{manningDefense = true;};
				case "true":{manningDefense = false;};
			};
...
while {!isNil "BIS_CONTROL_CAM"} do {
	...
	if (_mode in ["mousemoving","mouseholding"]) then {
		...
		if ((_tooltipType + _tooltip) != _oldTooltip || commandingMenu != _oldMenu || _logic getVariable "WF_RequestUpdate") then {
			...
			_filePicture = "";
			if !(_type in ["Sign_Danger"]) then {
				_filePicture = getText (configFile >> "cfgvehicles" >> _type >> "picture");

				_filePicture = if (_filePicture != "pictureStaticObject")then{ 
					if (str(configFile >> "CfgVehicleIcons" >> _filePicture) != "") then {
						getText (configFile >> "CfgVehicleIcons" >> _filePicture)
						}else{""}
				}else{""};
			};
			...
			//_textHelpInfo = "<t color='#42b6ff' size='1'>F1 for Help</t>";
			_text1 = if (count _params > 0) then {"<t color='#42b6ff' shadow='2'>" + localize "str_coin_rotate" + "<t align='right'>" + call compile (keyname 29) + "</t></t><br />"} else {"<br />"};
