...
if (_index != -1) then {
	...
	if (sideID == _captureSideID) then {
		...
		if (Leader Group player == player) then {
			...
			{if (_x distance _camp < 25) then {_award = true}} forEach _units;
		};

		if (player Distance _camp < 150 || _award) then {
			('WFBE_CAMPCAPTUREBOUNTY' Call GetNamespace) Call ChangePlayerFunds;		
			if (player Distance _camp < 25 || _award) then {
		...
		_markerCircle setMarkerColorLocal ("Friendly" Call GetMarkerColoration);
	} else {
		if (_townSideID == sideID || getMarkerColor _marker == ("Friendly" Call GetMarkerColoration)) then {
			...
			_markerCircle setMarkerColorLocal ("Enemy" Call GetMarkerColoration);
		};
	};
};
