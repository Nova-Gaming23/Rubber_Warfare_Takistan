...
//Side msg
_soldBySide = _structure getVariable ["WFBE_SOLD",nil];
if (isNil '_soldBySide') then {
	["Destroyed",_type,_side] Spawn SideMessage;
}else{
	//Double check side ?Destroyed in last second from enemy player?
	if (_soldBySide != _side) then {
		["Destroyed",_type,_side] Spawn SideMessage;
	};
};
