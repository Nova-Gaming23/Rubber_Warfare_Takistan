...
//Try to Get Admin ID´s From Server Mod
ExecVM "Server\Module\Admin\Get_AdminIDs.sqf";
...
//--- NG23 Score-Logger
diag_log Format["[WFBE (INIT)][frameno:%1 | ticktime:%2] Init_Server: Init NG23-ServerSide-Score-Module",diag_frameno,diag_tickTime];
WF_Logic setVariable ["NG23_ScoreLogger_Enabled",false,true];
ExecVM "Server\NG23_Score_To_File\Score_Logger.sqf";
...
//--- Fast Time.
if (PARM_Environment_FastTime > 0) then {
...
switch (PARM_Base_StartingLocations) do {
	...
	case 2: {
		...
		_attempts = 0;
		_ssd = PARM_Base_StartingDistance;

		//--- Random range?.
		if (_ssd == -1) then {_ssd = floor(random(9500))};
		_locationLogics = _locationLogics - [StartingLocation0];
		_locationLogics = _locationLogics - [StartingLocation1];

		while {_eastLocation distance _westLocation < _ssd && _attempts <= 500} do {
			_eastLocation = _locationLogics select (random (_total - 3));
			_westLocation = _locationLogics select (random (_total - 3));
...
if (WF_Debug) then {
	...
	_markerEast setMarkerTypeLocal "Depot";
	_markerEast setMarkerColorLocal "ColorRed";
	_markerWest = "WestSpawnPosition";
	createMarkerLocal [_markerWest,getPos _westLocation];
	_markerWest setMarkerTypeLocal "Depot";
	_markerWest setMarkerColorLocal "ColorBlue";
};	
diag_log Format["[WFBE (INIT)][frameno:%1 | ticktime:%2] Init_Server: Starting Location Mode (%3) - [Done]",diag_frameno,diag_tickTime,PARM_Base_StartingLocations];
...
//--- Remove Inventory 
	clearWeaponCargoGlobal EastMHQ;
	clearMagazineCargoGlobal EastMHQ;
	clearWeaponCargoGlobal WestMHQ;
	clearMagazineCargoGlobal WestMHQ;
...
['WFBE_West_AICommanderFunds',round(PARM_Economy_StartingFundsEast*1.5),true] Call SetNamespace;
['WFBE_East_AICommanderFunds',round(PARM_Economy_StartingFundsEast*1.5),true] Call SetNamespace;
...
EastMHQ setVehicleInit Format["['Headquarters','ColorGreen',[1,1],'','HQUndeployed',this,0.2,true,'Headquarters','ColorOrange',true,East] ExecVM 'Common\Common_MarkerUpdate.sqf';%1",_text];
processInitCommands;
...
{
	_VehicleClass = "";
	_Inventory = [];//["",[[["m16a4_acg",1],["m16a4_acg",1]],[]]]
	if (typeName _x == typeName []) then {_VehicleClass = _x select 0;_Inventory = _x select 1}else{_VehicleClass = _x};
	_vehicle = [_VehicleClass,(getPos EastMHQ),east,false] Call CreateVehi;
	[_vehicle,getPos EastMHQ,35,50,true,false,true] Call PlaceNear;
	clearWeaponCargoGlobal _vehicle;
	clearMagazineCargoGlobal _vehicle;
	if (count _Inventory > 0) then {
		{_vehicle addWeaponCargoGlobal _x;}ForEach (_Inventory select 0);
		{_vehicle addMagazineCargoGlobal _x;}ForEach (_Inventory select 1);
	};
	_vehicle addWeaponCargoGlobal["Cobalt_File",1];
	EastMHQ addWeaponCargoGlobal["Cobalt_File",1];
...
{	
	...
	_Inventory = [];//["",[[["m16a4_acg",1],["m16a4_acg",1]],[]]]
	if (typeName _x == typeName []) then {_VehicleClass = _x select 0;_Inventory = _x select 1}else{_VehicleClass = _x};
	_vehicle = [_VehicleClass,(getPos WestMHQ),west,false] Call CreateVehi;
	...
	_vehicle addWeaponCargoGlobal["Cobalt_File",1];
	WestMHQ addWeaponCargoGlobal["Cobalt_File",1];
	//[_vehicle] call Func_Common_InitCustomVehicle;
...
if (paramAI) then {
	...
	_i = 1;
	{
		if !(isNil "_x") then {
			...
			Call Compile Format ["EastSlot%1Funds = %2; publicVariable 'EastSlot%1Funds';",_i,PARM_Economy_StartingFundsEast];
	...
	_i = 1;
	{
		if !(isNil "_x") then {
			...
			_x setVariable ["queue",[]];
			Call Compile Format ["WestSlot%1Funds = %2; publicVariable 'WestSlot%1Funds';",_i,PARM_Economy_StartingFundsWest];
			[_x, false]	Call SetTeamAutonomous;
			[_x, ""] Call SetTeamRespawn;
			[_x, -1] Call SetTeamType;
			[_x, "towns"] Call SetTeamMoveMode;
	...
	//--- AI Supply Trucks.
	if (PARM_Economy_SupplySystem == 0) then {
...
if !(paramMoneyOnly) then {
	WF_Logic setVariable ["EastSupplies",PARM_Economy_StartingSupplyEast,true];
	WF_Logic setVariable ["WestSupplies",PARM_Economy_StartingSupplyWest,true];
};
...
_upArray = if (PARM_Gameplay_UpgradesEast == 1) then {[0,0,0,0,3,1,2,2,1,1,3,1,1,0,1,1,1,3]} else {'WFBE_UPGRADELEVELS' Call GetNamespace};
EASTUpgrades = _upArray;
PublicVariable 'EASTUpgrades';
_upArray = if (PARM_Gameplay_UpgradesWest == 1) then {[0,0,0,0,3,1,2,2,1,1,3,1,1,0,1,1,1,3]} else {'WFBE_UPGRADELEVELS' Call GetNamespace};
...
if (PARM_Base_BaseAreaLimit > 0) then {
