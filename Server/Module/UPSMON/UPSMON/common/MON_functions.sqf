...
MON_GetIn_NearestStatic = {
	...
	if (isNull _npc) exitWith{_unitsIn};
...
MON_CreateMine = {
	...
	if (isNull _soldier) exitWith {false};
	...
	/*	OLD FUnction not working in MP 
	...
	*/
	...
	//The "unit" command is not up to date in the MP. sometimes the (units _leader select x) value is zero because the unit is no longer alive but is still in the (units) array. 
	if (count (units _npc) > 0) then {
		{
			if (alive _x) then {
				_rnd = round (random 1);
				if (_rnd == 1) exitWith {
					_soldier = (units _npc) select _foreachindex;
				};
			};
		}foreach units _npc;
	};	
