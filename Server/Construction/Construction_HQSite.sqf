...
if (!_deployed) then {
	...
	if (PARM_Base_BaseAreaLimit > 0) then {
		_isEmptyBaseSlot = true;
		_areas = WF_Logic getVariable Format['%1Area',_sideText];
		_near = ([_position,_areas] Call SortByDistance)select 0;

		{Private "_x";
			_buildung = _x;			 
			if ((_buildung distance _position) < ('WFBE_BASEAREARANGE' Call GetNamespace)) exitWith {
				 _isEmptyBaseSlot = false;				
			};		
		}ForEach ((_side) Call GetSideStructures);
		
		if (_isEmptyBaseSlot) then {
			if ((_near distance _position) <= (('WFBE_BASEAREARANGE' Call GetNamespace))) then {
				_areas = _areas - [_near];
				WF_Logic setVariable [Format['%1Area',_sideText],_areas,true];
			};
		};
	};
	...
	if (_side == East) then {
		_MHQ setVehicleInit "this addEventHandler ['handleDamage',{ (_this select 2)*0.67 }];";
		_MHQ setVehicleInit "this setObjectTexture [0,""desert\btr_body_co.jpg""]";
		_MHQ setVehicleInit "this setObjectTexture [1,""desert\btr_body2_co.jpg""]";
		_MHQ setVehicleInit "this setObjectTexture [2,""desert\btr_hq_co.jpg""]";	
	} else {
		_MHQ setVehicleInit  "this addEventHandler ['handleDamage',{ (_this select 2)*0.60 }];";
		_MHQ setVehicleInit "this setObjectTexture [0,""desert\lavbody_coD.jpg""]";
		_MHQ setVehicleInit "this setObjectTexture [1,""desert\lavbody2_coD.jpg""]";
		_MHQ setVehicleInit "this setObjectTexture [2,""desert\lav_hq_co.jpg""]";
	};
	...
	_MHQ setVehicleInit Format["['Headquarters','ColorGreen',[1,1],'','HQUndeployed',this,0.2,true,'Headquarters','ColorOrange',true,%1] ExecVM 'Common\Common_MarkerUpdate.sqf';%2",_side,_text];
	processInitCommands;
