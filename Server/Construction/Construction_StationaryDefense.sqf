...
_manRange = if (count _this > 6) then {_this select 6} else {paramDefenseManRange};
...
if (_type == 'Concrete_Wall_EP1') then {
	_defense addEventHandler ['handleDamage',{getDammage (_this select 0)+(_this select 2)/25}];
	
};
if (_type == 'Land_Ind_SawMillPen') then {
	_defense addEventHandler ['handleDamage',{getDammage (_this select 0)+(_this select 2)/5}];
	
};


Call Compile Format ["_defense addEventHandler ['Killed',{[_this select 0,_this select 1,%1] Spawn UnitKilled}]",_side];
_defense addEventHandler ["Killed",{_this select 0 Spawn TrashObject}];

if (_defense EmptyPositions "gunner" > 0 && ((PARM_Base_AutoDefenses > 0) || _isAIQuery)) then {
	...
	if (_manned) then {
		...
		if (count _alives < PARM_Base_AutoDefenses || _isAIQuery) then {
			...
			if !(isNull _closest) then {			
				...
				[_defense,_side,_team,_manned,_isAIQuery,_manRange] Spawn HandleEmptyActivDefense;
			};			
		};
	};
};
