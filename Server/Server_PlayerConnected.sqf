...
// Player Vehicle Part
_Var = Format["WFBE_JIP_USER_%1_Veh",_uid];
_get2 = JIP_Handler_Logic getVariable _Var; 
if (isNil '_get2') then {
	JIP_Handler_Logic setVariable [_Var,[],true];
};
...
if (isNil '_get') exitWith {
	...
	*/
	[
	Format["WFBE_JIP_USER%1",_uid],
	[_uid,
	_slotIndex,
	0,
	_sideJoined,
	_sideJoined,
	if (mysql) then {round(time)} else {0},
	vehicleVarName (leader _team)
	],true] Call SetNamespace;
	
	Call Compile Format ["%1Funds = %2; publicVariable '%1Funds';",_team getVariable 'identification',missionnamespace getVariable (Format["PARM_Economy_StartingFunds%1",str _sideJoined])];
};
...
if (_sideOrigin != _sideJoined) then {
	_funds = missionnamespace getVariable (Format["PARM_Economy_StartingFunds%1",str _sideJoined]);
};
