...
mando_getmaptgt_setup = 
{
   ...
   if (_event == "onLoad") then
   {
      ...
      {   
         ...
         []spawn 
         {
            ...
            createMarkerLocal ["mk_mando_maptgt_min_rng", [0,0]];
            "mk_mando_maptgt_min_rng" setMarkerShapeLocal "ELLIPSE";
            "mk_mando_maptgt_min_rng" setMarkerColorLocal "ColorBlue";
            "mk_mando_maptgt_min_rng" setMarkerAlphaLocal 0.25;			
			_min_rng_mk = 0;
			_var = _unit getVariable "mando_min_rng";
			if (!isNil "_var") then
			{
				_min_rng_mk = _var;
			};				
            "mk_mando_maptgt_min_rng" setMarkerSizeLocal [_min_rng_mk, _min_rng_mk];
            ...
            while {!isNull (findDisplay 24000)} do
            {
               ...
               "mk_mando_maptgt_min_rng" setMarkerPosLocal (getPos _unit);
               Sleep 0.1;
            };            
         };
      };
   ...
   else
   {
      if (_event == "onUnload") then
      {
         ...
         deleteMarker "mk_mando_maptgt_min_rng";		 
   ...
   {   
      if (((_this select 1) select 1) == 0) then
      {
         if (!((_this select 1) select 4)) then
         {
			...
			if ((_unit distance (((_this select 1) select 0) ctrlMapScreenToWorld [(_this select 1) select 2, (_this select 1) select 3])) > _min_rng) then
			{
				_marker_name = "mk_mando_maptgt";
				if (getMarkerColor _marker_name == "") then
				{
			...
			{
				titleText["\n\nTarget within min range", "PLAIN DOWN"];
			};
         };
      };
   };
      ...
      [] spawn  
      {
         ...
         while {!mando_exit_hud} do
         {
            if (!isNull (UINameSpace GetVariable "mandotarget_disp") && (mando_hud_missile > -1)) then
            {
               ...
               if (((_ttypes select 0) == "GROUND") && (_weapon in ["GLT_KAB500SE_Launcher","GLT_GBU39_Launcher","GLT_GBU53_Launcher","GLT_AGM154A_Launcher","GLT_AGM154A1_Launcher","sfp_rbs15_launcher","GLT_MOAB_Launcher","HEXA_AASMLauncher","PRACS_JDAM_Launcher"])) then
               {
   ...
   if (mando_key_pressed == mando_keyspecial) then
   {
      ...
      if ((_type == "F16") || (_type == "EF2000")) then
      {
         ...
         if (!isNull (UINameSpace GetVariable "mandotarget_disp") && (mando_hud_missile > -1)) then
         {
            _weapon = (mando_missileidxbase select mando_hud_missile) select 1;
            _ttypes = vehicle player getVariable "mando_target_types";
            if (((_ttypes select 0) == "GROUND") && (_weapon in ["GLT_KAB500SE_Launcher","GLT_GBU39_Launcher","GLT_GBU53_Launcher","GLT_AGM154A_Launcher","GLT_AGM154A1_Launcher","sfp_rbs15_launcher","GLT_MOAB_Launcher","HEXA_AASMLauncher","PRACS_JDAM_Launcher","RKSL_stormshadow_Launcher"])) then
            {
               _continue = true;
            };
         };
