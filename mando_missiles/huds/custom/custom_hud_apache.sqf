...
_hud_ColorChange = {
	...
	(_display DisplayCtrl 69) CtrlSetTextColor (mando_hud_colors select 0) + [mando_hud_max_alpha];
	(_display DisplayCtrl 70) CtrlSetTextColor (mando_hud_colors select 0) + [mando_hud_max_alpha];
...
_hud_show = {
	...
	(_display DisplayCtrl 67) CtrlSetTextColor [1.0, 1.0, 1.0, 0.95];
...
if (((Vehicle Player) GetVariable "mando_ecm_time") > 0) then {
...
[_vehicle] Spawn {
	...
	while {!mando_exit_hud} do {
		...
		if (IsNull _display) then {
			...
			if ((getPos _vehicle select 2) < 80) then
			{
			...
			{
				for [{_i=30},{_i<45},{_i=_i+1}] do
				{
					(_display DisplayCtrl _i) CtrlSetTextColor [0,0,0,0];
				};
			};
