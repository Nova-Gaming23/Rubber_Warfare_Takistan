// Hud modes setup
   mando_hud_init_code = 
   {
	  ...
	  if (!(vehicle player isKindOf "sfp_fpl37")) then
	  {
   ...
   // Missile parameters for sfp_rbs24_launcher (mando_hud_missile = 5);
   mando_hud_misp = mando_hud_misp + [[
   vehicle player,
   "",
   [0,0,0],
   ...
   mando_missileidxbase = mando_missileidxbase + [[-1,"sfp_rbs24_launcher"]]; 
