...
[]spawn
{
   ...
   [mando_a10s, 0, 0, ["Vehicles", "REMOTE"], "MMA LGB Camera", _mcc_tv_script, _launch_pos, _cam_pos_angles, {(driver _plane == player) && ("BombLauncherA10" in weapons _plane)}, 0, _weapon_idx, _background_mfd, 1, -3]execVM mando_missile_path+"mcc\mando_mccallow_by_type.sqf";

   _weapon_idx = "cwr2_BombLauncher";
   [mando_a10s, 0, 0, ["Vehicles", "REMOTE"], "MMA LGB Camera", _mcc_tv_script, _launch_pos, _cam_pos_angles, {(driver _plane == player) && ("cwr2_BombLauncher" in weapons _plane)}, 0, _weapon_idx, _background_mfd, 1, -3]execVM mando_missile_path+"mcc\mando_mccallow_by_type.sqf";
   
   _weapon_idx = "MaverickLauncher";
   _mcc_tv_script = mando_missile_path+"tv\tv_types\mando_tv_agm65.sqs";
   [mando_a10s, 0, 0, ["Vehicles"], "MMA AGM65 Camera", _mcc_tv_script, _launch_pos, _cam_pos_angles, {(driver _plane == player) && ("MaverickLauncher" in weapons _plane)}, 0, _weapon_idx, _background_mfd, 1, -3]execVM mando_missile_path+"mcc\mando_mccallow_by_type.sqf";

   _weapon_idx = "cwr2_MaverickLauncher";
   _mcc_tv_script = mando_missile_path+"tv\tv_types\mando_tv_agm65_2.sqs";
   [mando_a10s, 0, 0, ["Vehicles"], "MMA AGM65 Camera", _mcc_tv_script, _launch_pos, _cam_pos_angles, {(driver _plane == player) && ("cwr2_MaverickLauncher" in weapons _plane)}, 0, _weapon_idx, _background_mfd, 1, -3]execVM mando_missile_path+"mcc\mando_mccallow_by_type.sqf";


// BIS A10
   [mando_a10s, 0, "", [4,6], [4,6], 25, 25, 20,[4,3,-2.5], mando_missile_path+"huds\mando_hud_a10.sqf", 0, 0, [[-7,"SidewinderLaucher_AH1Z"],[-6,"MaverickLauncher"]],0]execVm mando_missile_path+"units\mando_assignvehicle_by_type.sqf";
