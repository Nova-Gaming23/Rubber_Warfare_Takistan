...
[1.005, 0.755, 0.075,0.075*4/3]]];
	...
	if (isNil "mando_lrr_max_range") then
	{
	...
	//Added Laescher _Undifinedvariable
   ...
   if (isNil "mando_lrr_cam_pos") then
   {
	...
	_var = mando_lrr_unit getVariable "mando_lrr_units";
	if (isNil "_var") then
	{
		mando_lrr_unit setVariable ["mando_lrr_units", [], true];
	};
...
// Back color has no effect, use only pic if exist
	...
	// MFD Back
	{
	...
	} forEach [23901, 23902, 23903, 23904, 123, 124, 125, 126, 127,23918, 23919, 23921, 23922, 23924, 23925, 23926, 23927, 23950,23951,23952,23960, 23961];		 
	  ...
	  _i = 1;
	  {
		if (_i > count (mando_lrr_unit getVariable "mando_lrr_units")) then
		{
			(_display displayCtrl _x) ctrlSetText "N/A";
			(_display displayCtrl _x) ctrlEnable false;
		};
		_i = _i + 1;
	  } forEach [23918, 23927, 23960];
...
/*
			...
			// MFD Back
			{
				_pos = ctrlPosition ((findDisplay 23900) displayCtrl _x);
				_pos = [0.5 + ((_pos select 0)-0.5)*_scale, 0.5 + ((_pos select 1)-0.5)*_scale, 
					(_pos select 2)*_scale, (_pos select 3)*_scale];							
				((findDisplay 23900) displayCtrl _x) ctrlSetPosition _pos;
				((findDisplay 23900) displayCtrl _x) ctrlCommit 0;
			} forEach [23901, 23902, 23903, 23904, 123, 124, 23918, 23919, 23921, 23922, 23924, 23925, 23926, 23927, 23950,23951,23952,23960, 23961];
*/			
		  };
					...
					{						
						...
						if (!isNull _target) then
						{
							...
							if (_dist < mando_lrr_range) then
							{
								...
								_poss = [0.5 + ((_poss select 0)-0.5)*_scale, 0.5 + ((_poss select 1)-0.5)*_scale, 
										(_poss select 2)*_scale, (_poss select 3)*_scale];							
		  ...
		  [_display] spawn
		  {
			...
			if (count(mando_lrr_unit getVariable "mando_lrr_units") == 0) then
			{
				_target = mando_lrr_unit;
			}
			else
			{
			...
			while {!isNull _display} do
			{
				...
				if ((_old_target != _target) || (_old_fov != mando_lrr_fov)) then
				{
					...
					_old_target = _target;
					if (_target == mando_lrr_unit) then
					{
						_cam camSetTarget (mando_lrr_unit modelToWorld [0,100,1]);
					}
					else
					{
...
//				_zoom = floor (1/((atan(mando_lrr_fov)*2) / (atan(1)*2)));
				...
				if (count(mando_lrr_unit getVariable "mando_lrr_units") == 0) then
				{
					(_display displayCtrl 23950) ctrlSetStructuredText parseText 
						format ["Radar: %1<br />Current Range: %2 Km<br />Max Range: %3 Km<br />Launcher: L%4<br />Cam: %5<br />Zoom: %6x", 
								_radar, floor (mando_lrr_range/1000), floor (mando_lrr_max_range/1000), getText (configFile>>"cfgVehicles">>typeOf vehicle player >>"displayName"), _target, _zoom];
				}
				else
				{
					(_display displayCtrl 23950) ctrlSetStructuredText parseText 
				...
				_txt = "";
				for [{_i=0},{(_i< count(mando_lrr_unit getVariable "mando_lrr_units")) && (_i<3)},{_i=_i+1}] do
				{
				...
				if ((dayTime * 3600) > (_elapsed + _deltat)) then
				{
					...
					if (!isNull mando_lrr_target) then
					{
							...
							(alive mando_lrr_target)) then
						{
							...
							_ecm = mando_lrr_target getVariable "mando_ecm_on";
							if (!isNil "_ecm") then
							{
								if (_ecm) then
								{
									if ((random 100) < 30) then
									{
										...
										if (count (mando_lrr_unit getVariable "mando_lrr_units") == 0) then
										{
											mando_lrr_unit setVariable ["mando_current_target", mando_lrr_target, true];
										};
...
// Max Rad X = 0.968/2 * 3/4
						   ...
						   if (_target == mando_lrr_target) then
						   {
							...
							_posl = [0.5, 0.5, (_posl select 0)-0.5, (_posl select 1)-0.5];
								...
								if (_dist < mando_lrr_range) then
								{
									...
									_posd = [0.5 + ((_posd select 0)-0.5)*_scale, 0.5 + ((_posd select 1)-0.5)*_scale, 
										(_posd select 2)*_scale, (_posd select 3)*_scale];							
									
								};
							_posl = [0.5 + ((_posl select 0)-0.5)*_scale, 0.5 + ((_posl select 1)-0.5)*_scale, 
										(_posl select 2)*_scale, (_posl select 3)*_scale];								
						   };						   
						};
					 };
				...
				if (mando_lrr_state) then {
					if (isNull mando_lrr_target) then {
						...
						playSound "mando_radar2_lessVol";
					};
				};
