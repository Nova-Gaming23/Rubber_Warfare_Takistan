...
// Path for scripts 
if (isNil "mando_missile_path") then
{
...
if (isNil "mando_weapons_forbidden") then
{
	mando_weapons_forbidden = ["ACE_B61BombLauncher"];
};

if (isNil "mando_rearm_sources_classes") then
{
	// mando_rearm_sources_classes = ["TowingTractor", "MtvrReammo", "KamazReammo", "UralReammo_INS", "UralReammo_CDF", "WarfareReammoTruck_USMC","WarfareReammoTruck_RU","WarfareReammoTruck_Gue", "DFReammo_Base"];
    //mando_rearm_sources_classes = [WESTAIR,EASTAIR,WESTSP,EASTSP,"WarfareReammoTruck_USMC","MtvrReammo_DES_EP1","MtvrReammo","KamazReammo", "UralReammo_INS", "UralReammo_CDF", "UralReammo_TK_EP1", "WarfareReammoTruck_USMC","WarfareReammoTruck_RU","WarfareReammoTruck_Gue"];
    mando_rearm_sources_classes = ["MtvrReammo_DES_EP1","MtvrReammo","KamazReammo", "UralReammo_INS", "UralReammo_CDF", "UralReammo_TK_EP1", "WarfareReammoTruck_USMC","WarfareReammoTruck_RU"];
};

// Array with weapons considered laser designators
if (isNil "mando_lasers") then
{
	mando_lasers = ["Laserdesignator","Laserdesignator_mounted", "Kiowa_Laser", "MAF_nh90_laser","yup_Seahawk_Laser"];
};
...
mando_score_tanks = 0;
mando_score_ships = 0;
mando_score_bships = 0;
mando_score_static = 0;
mando_score_crews = 0;
mando_score_missiles = 0;

if (isNil "mando_no_score") then
{
	if(isClass (configFile >> "CfgPatches" >> "MMA_NoScore")) then
	{
		mando_no_score = true;
	};
};

// Fills up potential ils destinations
if (isNil "mando_ilss") then
{
	mando_ilss = [];
};
...
if (!isDedicated) then
{
	...
	_pos = getArray (_cfg  >> "ilsPosition");
	if (count _pos > 0) then
	{
		_pos = _pos + [0];
		_dir = ((getArray (_cfg  >> "ilsDirection")) select 0) atan2 ((getArray (_cfg  >> "ilsDirection")) select 2);
		_log = "logic" createVehicleLocal [0,0,0];
		_log setPos _pos;
		_log setVariable ["mando_ils_angh", _dir];
		_log setVariable ["mando_ils_angv", asin((getArray (_cfg  >> "ilsDirection")) select 1)];
		_log setVariable ["mando_ils_ealt", 3];
		_log setVariable ["mando_ils_pos", _pos];
		_log setVariable ["mando_ils_name", format["Airfield %1", count mando_ilss]];
		
		mando_ilss = mando_ilss + [_log];		
	};	
	
	_cfg =(configFile >> "cfgWorlds" >> WorldName >> "SecondaryAirports");
	for [{_i=0},{_i < count _cfg},{_i=_i+1}] do
	{
		_pos = getArray (_cfg Select _i >> "ilsPosition") + [0];
		_dir = (getArray (_cfg Select _i >> "ilsDirection") select 0) atan2 (getArray (_cfg Select _i >> "ilsDirection") select 2);
		_log = "logic" createVehicleLocal [0,0,0];
		_log setPos _pos;
		_log setVariable ["mando_ils_angh", _dir];
		_log setVariable ["mando_ils_angv", asin((getArray (_cfg select _i >> "ilsDirection")) select 1)];		
		_log setVariable ["mando_ils_ealt", 3];
		_log setVariable ["mando_ils_pos", _pos];
		_log setVariable ["mando_ils_name", format["Airfield %1", count mando_ilss]];
	...
	if ((worldName == "utes") && (side player == west)) then
	{
		_log = "logic" createVehicleLocal [0,0,0];
		_log setPos [1357.6, 894.6, 0];
		_log setVariable ["mando_ils_angh", 180];
		_log setVariable ["mando_ils_angv", 4];
		_log setVariable ["mando_ils_ealt", 15];
		_log setVariable ["mando_ils_pos", [1357.6, 894.6, 0]];
		_log setVariable ["mando_ils_name", "USS Keh Sanh"];
