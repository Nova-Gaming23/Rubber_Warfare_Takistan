   ...
   if (mando_hud_guidance == 4) then
   {
      [vehicle player] spawn
      {
         ...
         while {_continue} do
         {
			...
			if (speed _unit > 0 && (_posu select 2)>0) then {
...
// L CTRL, get target
   ...
   switch (_ttypes select 0) do
   {
      ...
      case "LOCATIONS":
      {
         ...
         {
			_text = "<t color='#ffffff' size='2.2' shadow='1'shadowColor='#999999'>MMA ILS</t><br />";
            ...
            if (mando_ils_selected >= (count mando_ilss)) then
            {
				...
				_text = _text + "<t size='1.5'><t align='left'>None</t></t><br />";
            }
			else
			{
				mando_ils_dest = mando_ilss select mando_ils_selected;			
				_text = _text + format["<t size='1.5'><t align='left'>%1 : %2 Km</t></t><br />", 
									mando_ils_dest getVariable "mando_ils_name", floor((mando_ils_dest distance vehicle player)/1000)];				
			};
			playSound "mando_target1";
			hintSilent parseText _text;	
			if (mando_hud_guidance != 4) then
			{			
				_unit setVariable ["mando_current_target", mando_ils_dest];
			};
         };
...
Sleep 0.1;
                  ...
                  if ((!isNull _mis) && (mando_hud_guidance != 4)) then
                  {
...
// titleText[ "No bis missile detected on time, virtual one used", "PLAIN"];
...
//            titleRsc["mandonomissiles", "PLAIN"];
	...
	if (((vehicle player getVariable "mando_target_types") select 0) == "LOCATIONS") then
	{
		if (mando_hud_guidance != 4) then
		{	
			[""]execVm"mando_missiles\huds\custom\custom_hud_waypoints.sqf";
		};
	};
};
