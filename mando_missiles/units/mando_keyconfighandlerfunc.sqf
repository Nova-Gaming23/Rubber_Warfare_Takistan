...
switch (_event) do
{
   ...
   case "onAccept":
   {
      ...
      mando_keyecmname  = mando_keyecmname_temp;
		...
		profileNamespace setVariable ["NG23_RubberWF_MandoKeys",
			[
				mando_keyfire,
				mando_keyflare,
				mando_keymode,
   ...
   case "onRepeat":
   {
	...
	case "RestoreDefault":{
		...
		//mando_keyfire
		mando_keyfire_temp = NG23_MandoKey_Defaults select 0;
		mando_keyfirename_temp = keyname (NG23_MandoKey_Defaults select 0);
		ctrlSetText [MISKEY_FIREKEY, format["%1", mando_keyfirename_temp]];
		
		//mando_keyflare
		mando_keyflare_temp = NG23_MandoKey_Defaults select 1;
		mando_keyflarename_temp = keyname (NG23_MandoKey_Defaults select 1);
		ctrlSetText [MISKEY_FLARESKEY, format["%1", mando_keyflarename_temp]];

		//mando_keymode
		mando_keymode_temp = NG23_MandoKey_Defaults select 2;
		mando_keymodename_temp = keyname (NG23_MandoKey_Defaults select 2);
		ctrlSetText [MISKEY_MODEKEY, format["%1", mando_keymodename_temp]];

		//mando_keyecm
		mando_keyecm_temp = NG23_MandoKey_Defaults select 3;
		mando_keyecmname_temp = keyname (NG23_MandoKey_Defaults select 3);
		ctrlSetText [MISKEY_ECMKEY, format["%1", mando_keyecmname_temp]];
