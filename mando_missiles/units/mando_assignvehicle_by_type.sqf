...
if (count _this == 2) exitWith {[_this select 0,_this select 1]execVm mando_missile_path+"NG23\SKIP_Turrent.sqf"};
...
_planetypes = [];
_planetypes = _planetypes + _valid;
      ...
      if ((vehicle player) != player) then
      {
         ...
         if ((count _planetypes) == 1) then
         {
            if (_class isKindOf ((_planetypes) select 0)) then
            {
               ...
               if (((_position == 0) && (_driver == player)) || ((_position == 1) && (_gunner == player)) || ((_position == 2) && (commander _plane == player))) then
               {
                  _inplane = true;
               };
            };  
