...
*/
		...
		if ((isNull _mis) && (count _this > 6)) then
		{
			_mis = _this select 6;
		};
...
// HINT format["Missile: %1 fired detected, class: %2\nVehicle class:%3", _mis, _ammo_class, typeOf _veh];
...
//   [101, 103, 108, 111, 113, 90, 91] call MandoScaleControls;   
         ...
         if (((abs(_angh) < 10) && (abs(_angv) < 10) && (_dist < mando_maxthrustedrange) && (_dist > mando_minimumrange)) || (mando_hud_guidance == 4)) then
         {
            ...
            else
            {
               if (mando_hud_guidance == 4) then
               {
                  ...
                  {
                     _rombe ctrlSetTextColor [0.90,0,0,1];
                  }
                  else
                  {
                     _rombe ctrlSetTextColor [0,0,0,0];
...
//            _type ctrlSetStructuredText parseText "Target: --- <br />Range: --- m <br />Altitude: --- m <br />TTI: --- s";
   ...
   if (!(alive _unit) || (isNull _unit) || ((_playerpos == 0) && (driver _unit != player)) || ((_playerpos == 1) && (gunner _unit != player)) || ((_playerpos == 2) && (commander _unit != player))) then
   {
      _continue = false;
      mando_exit_hud = true;
   };
