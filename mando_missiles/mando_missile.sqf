...
_mis = _type createVehicle [0,0,600];
_mis setDir _dirini;
_distanceini = _rtarget distance _log;


[_launcher, _mis, _boomscript, _rtarget, _proximity] spawn
{
	private["_launcher","_mis", "_boomscript", "_side", "_rtarget", "_proximity", "_posmg"];
	_launcher = _this select 0;
	_mis = _this select 1;
	_boomscript = _this select 2;
	_rtarget = _this select 3;
	_proximity = _this select 4;
	_side = side _rtarget;
	_posmg = getPos _mis;
/*	
	while {alive _mis} do
...
*/
	...
	while {alive _mis} do
	{
		...
		{
			_return = false;
			for [{_i=0},{(_i<20) && !_return},{_i=_i+1}] do
			{
				if (alive _mis) then
				{
					_posmg = getPos _mis;
...
/*
[_launcher, _mis, _boomscript, _side, _rtarget, _proximity] spawn
{
	private["_launcher","_mis", "_boomscript", "_side", "_rtarget", "_proximity", "_posmg"];
	_launcher = _this select 0;
	_mis = _this select 1;
...
_timeOut = time + 120;
// Workaround to stop spamming while Missile missing

while {!_finish && time <= _timeOut} do
{
         ...
         if ((!isNull _x) && (isNull _chaff)) then 
         {
            ...
            if (_dstchaff < _chaffrgn) then
            {
               if (_dstchaff < ((_mis distance _rtarget)+_chaffdelta)) then
               {
                  ...
                  {
					_devh = abs (((_mis worldToModel (getPos _x)) select 1) atan2 ((_mis worldToModel (getPos _x)) select 0));
					_devv = abs (((_mis worldToModel (getPos _x)) select 1) atan2 ((_mis worldToModel (getPos _x)) select 2));
					
					//hintSilent format["%1 %2", _devh, _devv];
					if ((_devh < 30) && (_devv < 30)) then
					{					
						_chaff = _x;
						_target = _chaff;
					};
...
//      _speed = _speedmax;
   ...
   if (!isNull _mis) then 
   {
      ...
      _posmg = getPos _mis;
		...
		diag_Log ["ERROR IN MANDO [_posm Can´t set] Millile is null",_this];
		_posmg = [0,0,0];
	};
...
// The missile reached target, cruise alt or a mid-course position
      ...
      else
      {
         ...
         {
			//DebugTest
			if (isNil '_posmg') then {
				diag_Log ["ERROR IN MANDO [_posmg Is Nil] Cant exit Whilt Loop",_this];
			};
