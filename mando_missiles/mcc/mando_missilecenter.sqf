...
for [{_i=0},{_i<100},{_i=_i+1}] do
{
	...
	if (!isNull ((findDisplay 21900) displayCtrl (21900+_i))) then
	{
		_pos = ctrlPosition ((findDisplay 21900) displayCtrl (21900+_i));
		_pos = [0.5 + ((_pos select 0)-0.5)*_scale, 0.5 + ((_pos select 1)-0.5)*_scale, 
			(_pos select 2)*_scale, (_pos select 3)*_scale];							
		((findDisplay 21900) displayCtrl (21900+_i)) ctrlSetPosition _pos;
		((findDisplay 21900) displayCtrl (21900+_i)) ctrlCommit 0;
		((findDisplay 21900) displayCtrl (21900+_i)) ctrlSetFontHeight 0.02*_scale;
	};
};
