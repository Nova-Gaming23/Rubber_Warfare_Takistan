...
if (isNil "mando_console_switch") then
{
   mando_console_switch = 
   {
      ...
      _exclude = [23932,23938,23903,23939,23940,23942,23943,23946,23948,23949,23950,23951,23952,23953,23954,23959,23960,23962,23963, 23999,23998,23997];
...
//      mando_support_display = _display;    
	...
	{
		_ctrl = _display displayCtrl (23900+_i);
		if (!isNull _ctrl) then
		{
			_pos = ctrlPosition _ctrl;
			...
			_ctrl ctrlCommit 0;
			_ctrl ctrlSetFontHeight 0.02*_scale;
		};
	};
	  ...
	  _pos = ctrlPosition (_display displayCtrl 23996);		
      (_display displayCtrl 23996) ctrlSetPosition [_pos select 0, _pos select 1, _pos select 2,0];
      (_display displayCtrl 23996) ctrlCommit 1;
      ...
      (_display displayCtrl 23997) ctrlSetText (mando_missile_path+"mando_bombs\laptop.paa");
...
*/					 
					 _scale = (safeZoneH/1.2549);
					 _ctrls = [23903, 23959, 23932];
					 _i =0;
					 {			
						 (_commdisplay displayCtrl (_ctrls select _i)) ctrlSetPosition [0.5 + ((_x select 0)-0.5)*_scale, 0.5 + ((_x select 1)-0.5)*_scale, (_x select 2)*_scale, (_x select 3)*_scale];
						 (_commdisplay displayCtrl (_ctrls select _i)) ctrlCommit 0;
						_i = _i + 1;
					 
					 } forEach [[1.5, 1.5, 0.75, 1-0.04],[0.007, 0.0015, 0.25*3/4, 0.25],[0, 0, 0.75, 1]];
                  }
                  else
                  {
/*				  
...
*/
					 ...
					 } forEach [[1.5, 1.5, 0.75, 1],[0, 0, 0.75, 1-0.04],[1.5, 1.5, 0.25*3/4, 0.25]];
...
_last_cam_attached = objNull;
   ...
   case "onMouseMoving":
   {
      ...
      if ((abs(_temp_angles select 0) <= _scale) && (abs(_temp_angles select 1) <= _scale)) then
      {
...
*/
            ...
            if (!isNil "mando_airsupport_code_pa") then
            {
               [_group, _group_i] spawn mando_airsupport_code_pa;
            };
