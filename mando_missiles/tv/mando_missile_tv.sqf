...
[1.005, 0.755, 0.075,0.075*4/3]]];
   ...
   {
      mando_tv_sensor_rng_w = 1500;
   };

   if (isNil "mando_tv_sensor_rng_l") then
   {
      mando_tv_sensor_rng_l = 3500;
   };
...
switch (_event) do
{
   ...
   case "onLoad":
   {
      ...
      {
         mando_last_missile_target = objNull;
      };
	 ...
	 _i = 0;
	 {
		(_display displayCtrl _x) ctrlSetPosition ((mando_tv_backcolor select 1) select _i);
		(_display displayCtrl _x) ctrlCommit 0;
		_i = _i + 1;
	 } forEach [23925,23926,23922,23921,23919,23918,23927,23960,23961];
	  
	_scale = (safeZoneH/1.2549);	  
	// MFD Back
	{
		_pos = ctrlPosition (_display displayCtrl _x);
		_pos = [0.5 + ((_pos select 0)-0.5)*_scale, 0.5 + ((_pos select 1)-0.5)*_scale, 
			(_pos select 2)*_scale, (_pos select 3)*_scale];							
		(_display displayCtrl _x) ctrlSetPosition _pos;
		(_display displayCtrl _x) ctrlCommit 0;
		(_display displayCtrl _x) ctrlSetFontHeight 0.02*_scale;		
	} forEach [23920, 23901, 23924, 23925,23926,23922,23921,23919,23918,23927,23960,23961, 123, 124];
...
*/		 
      ...
      []spawn
      {
         ...
         _cam = "camera" camcreate (mando_tv_plane modelToWorld mando_tv_cam_pos);
...
// mando_cam_angles = [0,0];
            _old = objNull;
            mando_last_missile = objNull;
			_cam_tgt = "logic" createVehicleLocal (mando_tv_plane modelToWorld [0,8000,0]);
			_old_scale = 0;
			_old_state = 0;
			_first = true;
            waitUntil
               ...
               if (mando_tv_cam == 1) then
               {
                  if (!isNull mando_last_missile) then
                  {
                     if (_old != mando_last_missile) then
                     {
						...
						_cam camSetTarget _cam_tgt;
						_first = true;						
                     };
                     _cam camSetPos (mando_last_missile modelToWorld [0,15,0]);
//                     _cam camSetTarget (mando_last_missile modelToWorld [0,50,0]);
					_cam_tgt setPos (mando_last_missile modelToWorld [0,50,0]);
					if ((_old_scale != mando_tvcam_scale) || _first) then
					{
						 _cam camSetFov mando_tvcam_scale;
						 _cam camCommit 0;
						 _old_scale = mando_tvcam_scale;
						 _first = false;
					};
                  }
                  else
                  {
					...
					mando_cam_angles = [(mando_cam_angles select 0)+(mando_tvcam_angles select 0), mando_cam_angles select 1];

					if (abs((mando_cam_angles select 1)+(mando_tvcam_angles select 1)) < 89) then
					{
						mando_cam_angles = [mando_cam_angles select 0, (mando_cam_angles select 1)+(mando_tvcam_angles select 1)];
					};


					if ((mando_cam_angles select 0) > 360) then
					{
						mando_cam_angles = [(mando_cam_angles select 0) - 360, mando_cam_angles select 1];
					}
					else
					{
						if ((mando_cam_angles select 0) < 0) then
						{
							mando_cam_angles = [360 + (mando_cam_angles select 0), mando_cam_angles select 1];
						}
					};
...
//                           _pos = [(getPos mando_tv_plane select 0)+sin(mando_cam_angles select 0)*200000*cos(mando_cam_angles select 1), (getPos mando_tv_plane select 1)+cos(mando_cam_angles select 0)*200000*cos(mando_cam_angles select 1), (getPos mando_tv_plane select 2)+sin(mando_cam_angles select 1)*200000];
							_cam_tgt setPosASL [(getPosASL mando_tv_plane select 0)+sin(mando_cam_angles select 0)*200000*cos(mando_cam_angles select 1), 
												(getPosASL mando_tv_plane select 1)+cos(mando_cam_angles select 0)*200000*cos(mando_cam_angles select 1), 
												(getPosASL mando_tv_plane select 2)+sin(mando_cam_angles select 1)*200000];
//                           _cam camSetTarget _pos;
                        ...
                        else
                        {
							...
							if (typeName mando_tv_cam_target == "Array") then
							{
								_cam_tgt setPos mando_tv_cam_target;
							}
							else
							{
								_cam_tgt setPosASL getPosASL mando_tv_cam_target;
							};
//                           _cam camSetTarget mando_tv_cam_target;
               ...
               "mk_mma_tv_dest" setMarkerPosLocal mando_tv_dest;
				if (isNull _commdisplay) then
				{
					true;
				};
            };
			
			deleteVehicle _cam_tgt;
               ...
               if (mando_tv_cam == 1) then
               {
                  if ((alive mando_tv_plane) || (alive mando_last_missile)) then
                  {
                     ...
                     if (!isNull mando_last_missile) then
                     {
                        ...
                        _texthelp ctrlSetStructuredText parseText "RDY";
...
//                                          mando_tv_laserlogic setPos [getPos _loglos select 0, getPos _loglos select 1, 1.5];
                                          mando_tv_laserlogic setPos [getPos _loglos select 0, getPos _loglos select 1, 0];
                                       };
...
*/
			...
			if (!isNil 'mando_ppccor') then {ppEffectDestroy mando_ppccor;};
			if (!isNil 'mando_ppcinv') then {ppEffectDestroy mando_ppcinv;};
			if (!isNil 'mando_ppdyblur') then {ppEffectDestroy mando_ppdyblur;};
			if (!isNil 'mando_ppfgrain') then {ppEffectDestroy mando_ppfgrain;};	
         };
   ...
   case "onMouseMoving":
   {
      ...
      {
		 _sensitivity = 0.7;//Original 0.2
         _temp_angles = [(((_this select 1) select 1)-0.5)/_sensitivity,(0.5-((_this select 1) select 2))/_sensitivity];
...
//            _target setPos (screenToWorld [0.5, 0.5]);
			_target setPos (positionCameraToWorld [0,0,10000]);
			
            [_target] spawn
            {
               private["_elapsed", "_mis", "_pos", "_target"];
               _elapsed = (dayTime * 3600);
			   _mis = objNull;
               ...
               while {(dayTime * 3600) < (_elapsed + 15)} do
               {
                  if (!isNull mando_tv_laserlogic_target) then
                  {
                     _target setPosASL (getPosASL mando_tv_laserlogic_target);
                  }
                  else
                  {
						if (isNull _mis) then
						...
						{						
						  ...
						  {
							_target setPos positionCameraToWorld [0,0,(mando_tv_plane distance _mis)+50];
						  };
						};
                  };
                  Sleep 0.01;
                     ...
                     if (!isNull _mis) then
                     {
                        [_mis, _missparam]spawn
                        {
                           ...
                           else
                           {
				   		   ...
				   		   _mis setPos [getPos _mis select 0,getPos _mis select 1,31000];
                           [_mis] spawn
                           {
