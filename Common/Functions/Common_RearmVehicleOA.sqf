...
//Tungi
				if (_vehicle isKindOf "2S6M_Tunguska") then {
					_vehicle removeWeapon "9M311Laucher";
...
//Rearm TOR
				if (_vehicle isKindOf "ZSU_TK_EP1") then {
					_vehicle setVariable ["mando_missilesleft0", 6, true];
				};
				
				//Rearm Rapier
				if (_vehicle isKindOf "ZSU_CDF") then {
					_vehicle setVariable ["mando_missilesleft0", 6, true];
				};