...
for [{_i = 0},{_i < (count (missionConfigFile/"Params"))},{_i = _i + 1}]  do {
	...
	if (_paramName == "PARM_Gameplay_LocationLayout") exitWith {_index =_i};
};

_value = if (isMultiplayer) then {paramsArray select _index} else {getNumber (missionConfigFile >> "Params" >> "PARM_Gameplay_LocationLayout" >> "default")};
...
switch (_value) do {
	...
	case 5: {TownTemplate = WF_Logic getVariable "Towns_RemovedMisak"};
	//--- case 4 = full. useless.
